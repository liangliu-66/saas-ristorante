...
export default function PromotionsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [promoRes, discRes] = await Promise.all([
        supabase.from('promotions').select('*').eq('restaurant_id', restData.id).order('created_at', { ascending: false }),
        supabase.from('discount_rules').select('*').eq('restaurant_id', restData.id).order('min_amount', { ascending: true })
      ]);
  ...
  const handleAddPromo = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!promoTitle || isSubmitting) return;
    ...
    if (!error) {
      ...
      alert(`Errore inserimento promozione: ${error.message}`);
    }
  };
  ...
  const handleAddDiscount = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!minAmount || !discountPercent || isSubmitting) return;
    ...
    if (!error) {
      ...
      alert(`Errore inserimento sconto: ${error.message}`);
    }
  };
          ...
          <form onSubmit={handleAddPromo} className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-slate-900 p-3 rounded-lg border border-slate-700 text-xs">
            ...
            <div className="flex gap-2">
              ...
              <button 
                ...
                className="bg-amber-500 hover:bg-amber-600 text-slate-900 font-bold px-4 py-2 rounded shrink-0 disabled:opacity-50"
              >
                {isSubmitting ? '...' : 'Aggiungi'}
              </button>
            </div>
          </form>
